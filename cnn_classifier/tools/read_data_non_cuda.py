...
class ChestXrayDataSet(Dataset):
    def __init__(self, image_list_file, transform=None, combine_pneumonia=False):
        ...
        if combine_pneumonia:
            ...
            if (torch.cuda.is_available()):
                ...
                self.loss_weight_minus = torch.FloatTensor([self.num_normal, self.num_pneumonia, self.num_covid]).unsqueeze(0).to(torch.device("cpu"))
            ...
            if (torch.cuda.is_available()):
                ...
                self.loss_weight_minus = torch.FloatTensor([self.num_normal, self.num_bact, self.num_viral, self.num_covid]).unsqueeze(0).to(torch.device("cpu")) / self.total
            self.loss_weight_plus = 1.0 - self.loss_weight_minus
    ...
    def loss(self, output, target):
        ...
        if (torch.cuda.is_available()):
            ...
            weight_plus = torch.autograd.Variable(self.loss_weight_plus.repeat(1, target.size(0)).view(-1, self.loss_weight_plus.size(1)).to(torch.device("cpu")))
            weight_neg = torch.autograd.Variable(self.loss_weight_minus.repeat(1, target.size(0)).view(-1, self.loss_weight_minus.size(1)).to(torch.device("cpu")))
        loss = output
